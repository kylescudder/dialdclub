...
There is no iOS test target, linter, or formatter configured. The Astro site has `format` and `format:check` scripts under `Site/package.json`; don't invent additional validation commands.
...
- Supabase Edge Functions require `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` in the Supabase dashboard. The service-role key must never be shipped in either app target. TestFlight signing and the production xcconfig come from GitHub Actions secrets in `.github/workflows/ios-testflight.yml`.
...
- On sign-in, `AppServices.applyAuth` synchronizes StoreKit entitlements and calls `refreshAll`; that refreshes profile, beans, brews, stats, APNs registration, local reminders, and the widget snapshot. On sign-out it resets only the in-memory subscription state.
- New brew creation must continue through `AppServices.canCreateNewExtraction()`: non-subscribers may create five active extractions, while edits remain allowed. Successful brew mutations should refresh brew data through `refreshBrewData()` so stats and WidgetKit stay in sync.
...
- Postgres schema lives in ordered files under `supabase/migrations/`. `profiles`, `beans`, `brew_sessions`, `brew_steps`, `brew_reminders`, and `device_tokens` are user-owned and protected by RLS. `iap_entitlements` is readable only by its user and written by service-role Edge Functions.
