...
- The free limit is `AppServices.freeExtractionLimit == 5`. Migration `0006_enforce_free_extraction_limit.sql` seeds and atomically increments `extraction_creation_quotas` for every genuine insert, so soft or hard deletion never restores allowance. Any change to the limit or what constitutes an extraction must update the database trigger, status RPC, client gate, tests, and user-facing paywall copy together.
